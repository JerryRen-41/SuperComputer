...
struct Problem {
    int M{ 400 }, N{ 600 };                
    double A1{ -1.2 }, B1{ 1.2 };          
    double A2{ -1.2 }, B2{ 1.2 };          
...
#pragma omp parallel for collapse(2) schedule(static)
        ...
        if ((norm <= eps_rel * norm0) || (st.dxE < delta)) {
            st.resE = norm;
            break;
        }

        applyDinv(P, r, z);
        double rz = dotE(P, r, z);
...
int main(int argc, char** argv) {
    ...
    std::vector<int> threads_list = { 1, 4, 16 };

    cout << "\n>>> Batch run for grid 40x40:\n";
    ...
    for (int t : threads_list) {
        auto st = run_case(40, 40, t, /*report_every=*/0, /*save_solution=*/true);
        cout << std::left
            << std::setw(12) << "40x40"
            << std::setw(10) << t
